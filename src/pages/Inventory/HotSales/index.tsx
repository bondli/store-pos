...
import useHotSalesColumns from './columns';

const NoStockList: React.FC = () => {
  const { message } = App.useApp();
  const { currentLang } = useContext(MainContext);

  const columns = useHotSalesColumns();
  ...
  const getNoStockList = async (t) => {
    ...
    try {
      const response = await request.get('/inventory/getHotSalesList', {
        params: t,
      });
  ...
  return (
    <>
      <Button
        ...
        onClick={togglePanel}
      >
        {language[currentLang].inventory.hotSalesAction}
      </Button>
      <Drawer
        title={`${language[currentLang].inventory.hotSales}`}
        width={1000}
        open={showPanel}
